            ...
            setSpread(res.data.result.map((item, index) => {
                ...
                return (
                    <SpreadRoom key={index} onClick={goDetail}>
                        <img src={rooms[i].image} alt="around" />
                        <div style={{ width: '100%' }}>
                            <div style={{ display: 'flex', width: '100%', justifyContent: 'space-between' }}>
                                <div>
                                    <p style={{ fontSize: '1.15vw', color: '#959595' }}>{`${item.address}의 ${item.stayType}`}</p>
                                    <p style={{ marginTop: '1vw', fontSize: '1.4vw', width: '25vw', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', height: '2vw' }}>{rooms[i].explain}</p>
                                </div>
                            ...
                            <div style={{ marginTop: '1vw', fontSize: '1.1vw', color: '#959595' }}>
                                <p>{`최대 인원 ${item.maxGuests}명 · 침실 ${item.bedRoomCount}개 · 침대 ${item.bedCount}개 · 욕실 ${item.showerRoomCount}개`}</p>
                            ...
                            <div style={{ display: 'flex', fontSize: '1.1vw', marginTop: '6vw', alignItems: 'center' }}>
                                ...
                                <p style={{ marginRight: '0.2vw' }}>{item.avgRate === null ? '0.0' : item.avgRate}</p>
                                <p style={{ color: '#b5b5b5' }}>{`(후기 ${item.cntRate === null ? '0' : item.cntRate}개)`}</p>
                            </div>
                        </div>
                    </SpreadRoom>
                );
